...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        # simply access the body of the operator function and for each operand, get its value
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        args_env = dict(zip(self.parameters, arguments)) # zip the expected parameters to the given arguments in a dictionary
        return self.body.eval({**self.parent, **args_env})  # remember to apply the parent environment first, and then the arguments
                                                            # env, since the parameter name can be the same as the parent function
