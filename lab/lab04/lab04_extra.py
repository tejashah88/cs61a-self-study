...
def merge(lst1, lst2):
    ...
    # easy cheesy 'weak-boi' approach
    # return sorted(lst1 + lst2)

    # iterative approach
    merged = lst1 + lst2
    new_merged = []
    if merged == lst1 or merged == lst2:
        return merged

    while merged:
        low_num = min(merged)
        new_merged.append(low_num)
        merged.remove(low_num)
    return new_merged
    ...
    while lst1 and lst2:
        if lst1[0] < lst2[0]:
            final_lst.append(lst1[0])
            lst1 = lst1[1:]
        else:
            final_lst.append(lst2[0])
            lst2 = lst2[1:]

    return final_lst + (lst1 if lst1 else lst2)
    """
    ...
    else: # doesn't matter for when lst1[0] == lst2[0]; either branch works
...
def replace_elem(lst, index, elem):
    ...
    new_lst = lst[:] # make a copy of the list
    new_lst[index] = elem
    return new_lst
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    def available_row(board, column, max_rows):
        for row in range(max_rows - 1, -1, -1):
            if get_piece(board, row, column) == '-':
                return row
        return -1

    new_row_index = available_row(board, column, max_rows)
    if new_row_index == -1:
        return -1, board
    new_row = replace_elem(board[column], new_row_index, player)
    new_board = replace_elem(board, column, new_row)
    return new_row_index, new_board
...
def print_board(board, max_rows, max_cols):
    ...
    for col in range(len(board) - 1, -1, -1): # iterate in reverse
        print(*board[col][::-1]) # reverse each row and unpack for printing
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    diagonal_win = check_win_diagonal(board, max_rows, max_cols, num_connect,
                                      row, col, player)
    return column_win or row_win or diagonal_win
