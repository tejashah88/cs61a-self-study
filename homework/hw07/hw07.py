...
def digits(n):
    ...
    """
    # (mostly) copy-paste from Lab 7
    s = Link.empty
    while n > 0:
        n, last = n // 10, n % 10
        s = Link(last) if s is Link.empty else Link(last, s)
    return s
...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        if not message.startswith(magic_word):
            ...
            # trim out the magic word
            command = message[len(magic_word)::]
            if hasattr(self.obj, command):
                cmd_fn = getattr(self.obj, command)
                return cmd_fn(*args)
            else:
