...
; Q9 tests: basic lambdas
(lambda (tom jerry) (+ tom jerry))
; expect (lambda (tom jerry) (+ tom jerry))

(define cool-lambda (lambda (a b) (lambda (x y) (+ a b x y))))
cool-lambda
; expect (lambda (a b) (lambda (x y) (+ a b x y)))
(cool-lambda 1 4)
; expect (lambda (x y) (+ a b x y))
((cool-lambda 1 4) 2 3)
; expect 10


; Q10 tests: shorthand lambda creation
(define (f x) (lambda (y) (+ x y)))
; expect f
((f 3) 6)
; expect 9

(define (useless-rtrn arg) arg)
; expect useless-rtrn
(useless-rtrn 'hello)
; expect hello


; Q11/Q12 tests: frame variable bindings
...
(define (hypotenuse-len x y) (sqrt (sum-squares x y)))
(hypotenuse-len 3 4)
; expect 5.0


; Q13 tests: and / or
(and True #t (not False) 666)
; expect 666
...
; Q14 tests: cond
; a testing function that increments 'n' by 1 before returning if the new 'n' equals v
(define (manipulative-tester n v) (define n (+ n 1)) (= n v))
(manipulative-tester 2 2)
; expect #f
(manipulative-tester 2 3)
; expect #t

(define x 3)

(cond
  ((manipulative-tester x 1) 'aye)
  ((manipulative-tester x 2) 'aye-aye)
  ((manipulative-tester x 4) 'aye-aye-aye-aye)
  (else 'darn))
; expect aye-aye-aye-aye
