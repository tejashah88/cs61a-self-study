...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 1
        return read_tail(src) # going into the parenthesis forest
        # END PROBLEM 1
    elif val == "'":
        # BEGIN PROBLEM 7
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            src.remove_front()
            return nil  # exiting from the parenthesis forest
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2
            src.remove_front() # remove the dot
            result = scheme_read(src) # parse the rest of the src
            if src.current() != ')': # if after scheme_read, we aren't at a closing paren, we have an invalid dotted pair
                raise SyntaxError
            ...
            # BEGIN PROBLEM 1
            # first, read the embedded expression, then read the rest
            # of the outer expression and return a pair as the result
