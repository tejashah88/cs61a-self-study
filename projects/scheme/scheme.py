...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        if symbol in self.bindings: # found the symbol, return the value associated with it
            return self.bindings[symbol]
        elif self.parent is not None: # we got a parent, ask it if it has a symbol in its 'bindings'
            return self.parent.lookup(symbol)
        # no symbol found and we are at the global frame at this point => raise an error
        # END PROBLEM 3
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        # the env param given isn't needed, since the frame is
        # a child of the same frame that the lambda lives in
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        # handling lambda short-hand format
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        var_pair = bindings.first
        check_form(var_pair, 2, 2)
        # remember: with curr_value, we have to evaluate it, or else the unevaluated form is stored in curr_formal
        curr_formal, curr_value = var_pair.first, scheme_eval(var_pair.second.first, env)
        formals, values = Pair(curr_formal, formals), Pair(curr_value, values)
...
def read_eval_print_loop(next_line, env, interactive=False, quiet=False, startup=False, load_files=()):
